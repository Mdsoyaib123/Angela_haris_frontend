    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-5 w-full">
      {statusData.map((single, index) => {
        ...
        return (
          <div
            key={single.title}
            className="w-full h-49.5 p-5 sm:p-6 bg-white rounded-2xl flex flex-col justify-between shadow-sm space-y-4"
          >
            {/* Top Row */}
            <div className="space-y-2">
              <div className="bg-[#F9F8F6] border-[#F6F4F2] w-12 h-12 rounded-xl p-3 flex items-center justify-center">
                <img
                  src={single.icon}
                  alt={single.title}
